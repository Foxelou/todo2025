...
class TodosValidationTest extends TestCase
{
    ...
    public function test_texte_est_obligatoire()
    {
        ...
        $response->assertSessionHas('message', 'Veuillez saisir une note à ajouter');
    ...
    public function test_texte_doit_avoir_une_longueur_minimale()
    {
        ...
        // $response->assertSessionHasErrors('La note doit contenir entre 3 et 256 caractères.');
        $response->assertSessionHas('message', 'La note doit contenir entre 3 et 256 caractères.');
        $this->assertDatabaseCount('todos', 0);
    }

    public function test_texte_doit_avoir_une_longueur_maximale()
    {
        $longTexte = str_repeat('a', 257); // 256 caractères
